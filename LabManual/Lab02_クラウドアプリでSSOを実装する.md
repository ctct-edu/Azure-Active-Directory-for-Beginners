...
5. 画面が遷移します。「無料トライアル版の利用を同意して開始」をクリックします。

   

6. 画面が遷移します。「お勤めの会社の規模は？」等、いくつか初期設定をする画面が表示されますが、「後にする」や「スキップ」「後にする」「ステップをとばす」などをクリックしたり、任意に回答します。

   > 注：この設定は、演習に影響しません。

   

7. 画面が遷移します。左メニューにある「管理コンソール」をクリックします。

      ![module03-05-5](./media/module03-05-5.BMP)

8. 左メニューにある「設定」をクリックした後、「セキュリティ」タブをクリックします

   ![module03-11](./media/module03-11.BMP)

9. 下の方にスクロールし「SSO ログインの URL」の「リンクをコピー」をクリックし、メモ帳などに貼り付けます。
...
18. 「Dropbox Business | SAML ベースのサインオン」の「Dropbox Business のセットアップ」にある「ログイン URL」の値をコピーしメモ帳などに貼り付けます。
...
1. 先ほど表示していた、Dropbox Businessの「設定＞セキュリティ＞シングル サインオン」画面に戻ります
